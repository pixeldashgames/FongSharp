...
module Game =
    ...
    let isValidPosition board row col =
        row >= 0
        && row < Array2D.length1 board
        && col >= 0
        && col < Array2D.length2 board
    ...
    let rec countAdjacentMines (board: Cell[,]) row col i =
        if i < 0 then
            ...
            let isValid = isValidPosition board new_row new_col
            let mineCount = if isValid && board[new_row, new_col].hasMine then 1 else 0

            mineCount + countAdjacentMines board row col (i - 1)
...
module GameFunctions =
    ...
    let rec discoverCells (board: Cell[,]) row col =
        ...
        if mutableBoard[row, col].hasMine then
            mutableBoard
        else if mutableBoard[row, col].adjacentMines > 0 then
            ...
            for j = 0 to 7 do
                ...
                if
                    isValidPosition mutableBoard new_row new_col
                    && mutableBoard[new_row, new_col].discovered = false
                then
    ...
    let leftClick (board: Cell[,]) row col =
        ...
        else if board[row, col].discovered then board
        else discoverCells board row col
